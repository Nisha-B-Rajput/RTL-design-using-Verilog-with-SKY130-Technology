...
In this session, I've performed simulation of multiplexer. I've added both the RTL design code and test bench code in iverilog to generate vcd file which I used in gtkwave generator to get the output waveforms after simulation. The output was generated by taking the inputs from the testbench code. 
...
But again, why do we have faster cells? Are faster cells sufficient?
**This complete collection forms .lib**

**Faster Cells vs Slower Cells**: 
Load in digital circuit is of **Capacitance**. Faster the charging or dicharging of capacitance, lesser is the cell delay. However, for a quick charge/ discharge of capacitor, we need transistors capable of sourcing more current i.e, we need WIDE TRANSISTORS. 

Wider transistors have lesser delay but consume more area and power. Narrow transistors give more delay but occupy less area and consume less power.Faster cells do not come free,they come at penalty of power and area.

**Selection of the Cells**: We'll need to guide the Synthesizer to choose the flavour of cells that is optimum for implementation of logic circuit.More use of faster cells results in a bad circuit in terms of power and area and also hold time violations arise.whereas more use of slower cells results in a sluggish circuit and may not meet the performance need. Keeping in view of previous observations of faster vs slower cells,to avoid hold time violations, larger circuits, sluggish circuits, we offer guidance to synthesizer in the form of **Constraints**.
...
Below image is the delay order for the different flavour of gates.
...
The synthesizer considers the module hierarchy and does the mapping accordting to instantiation. Here is the hierarchical netlist code for the  multiple_modules:
